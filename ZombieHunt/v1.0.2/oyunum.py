...
def Oyun():
    ...
    while True:
        ...
        altbar.ciz(ekran)
        ## GUI start
        Buttonum("Sarjor Arttır",10,505,170,40,red,"sarjor")
        Buttonum("+50 Can",10,555,170,40,red,"can")
        Buttonum("Mermi Hasarı",190,505,190,40,red,"hasar")
        if kare.can > 75:
        ...
        for zmbi in zombi_listesi:
            zombi_x =   kare.x_kor-zmbi.x_kor+25
            zombi_y =  kare.y_kor-zmbi.y_kor+25
            ...
            print(zmbi.can)
            try:
                if kare.x_kor  <= zmbi.x_kor+zmbi.r <= kare.x_kor+50 and kare.y_kor  <= zmbi.y_kor+zmbi.r <= kare.y_kor+50:
                    kare.can -= 15
...
def Buttonum(text,x_kor,y_kor,gen,yuk,renk,eventim):
    pos = pygame.mouse.get_pos()
    click = pygame.mouse.get_pressed()
    pygame.draw.rect(ekran,renk,[x_kor,y_kor,gen,yuk])
    text, text_kare = text_objesi(text,fontum)
    text_kare.center=(x_kor+gen/2,y_kor+yuk/2)
